...
[![Documentation](https://img.shields.io/badge/Docs-VitePress-646cff?style=for-the-badge&logo=vite&logoColor=white)](https://Wrapdf.com/docs/)

Visit our [Documentation](https://Wrapdf.com/docs/) for:
...
- **High Performance**: Built with modern web technologies, Wrapdf is fast and efficient, handling even large PDF files with ease.
- **Completely Free**: Wrapdf is a free and open-source tool for everyone.

---

## 🛠️ Features / Tools Supported

Wrapdf offers a comprehensive suite of tools to handle all your PDF needs.
...
Wrapdf is available in multiple languages:
...
Want to help translate Wrapdf into your language? Check out our [Translation Guide](TRANSLATION.md)!

---

## 🚀 Getting Started

You can run Wrapdf locally for development or personal use.
...
You can run Wrapdf directly from Docker Hub or GitHub Container Registry without cloning the repository:

You can also watch the video on how to set it up 👉
[Wrapdf Docker Setup](https://drive.google.com/file/d/1C4eJ2nqeaH__1Tlad-xuBHaF2Ha4fSBf/view?usp=drive_link)
...
This is the fastest way to try Wrapdf without setting up a development environment.
...
Since Wrapdf is fully client-side, all processing happens in the user's browser and no server-side processing is required. This means you can host Wrapdf as simple static files on any web server or hosting platform.
...
Wrapdf supports different compression modes for optimized builds:

```bash
# Gzip only (smallest Docker image size)
npm run build:gzip
docker build --build-arg COMPRESSION_MODE=g -t Wrapdf:gzip .

# Brotli only (best compression ratio)
npm run build:brotli
docker build --build-arg COMPRESSION_MODE=b -t Wrapdf:brotli .

# No compression (fastest build time)
npm run build:original
docker build --build-arg COMPRESSION_MODE=o -t Wrapdf:original .

# All formats (default, maximum browser compatibility)
npm run build:all
docker build --build-arg COMPRESSION_MODE=all -t Wrapdf:all .
```
...
Wrapdf can use jsDelivr CDN to serve large WASM files (LibreOffice, Ghostscript, PyMuPDF) for improved performance and reduced bandwidth costs:
...
Wrapdf can also be hosted from a subdirectory (e.g., `example.com/tools/Wrapdf/`):

```bash

# Example:
# 1. Build the app with the specific BASE_URL. BASE_URL must have a trailing and leading slash. The BASE_URL can be any url of your choice. Here we are using /tools/Wrapdf/ as an example.

BASE_URL=/tools/Wrapdf/ npm run build
...
mkdir -p serve-test/tools/Wrapdf

# 3. Copy all files from the 'dist' folder into that nested directory
cp -r dist/* serve-test/tools/Wrapdf/

# 4. Serve the 'serve-test' folder
npx serve serve-test
```

The website can be accessible at: ```http://localhost:3000/tools/Wrapdf/```
...
Wrapdf's Docker image also supports the `BASE_URL` build argument for subdirectory deployments:

```bash
# Build for subdirectory deployment
docker build --build-arg BASE_URL=/Wrapdf/ -t Wrapdf .

# Run the container
docker run -p 3000:8080 Wrapdf

# The app will be accessible at http://localhost:3000/Wrapdf/
```
...
> **Important**: 
> - Always include trailing slashes in `BASE_URL` (e.g., `/Wrapdf/` not `/Wrapdf`)
...
For organizations that want a clean, distraction-free interface focused solely on PDF tools, Wrapdf supports a **Simple Mode** that hides all branding and marketing content.
...
Wrapdf runs as a non-root user using nginx-unprivileged for enhanced security:
...
4. **Note your worker URL** (e.g., `https://Wrapdf-cors-proxy.your-subdomain.workers.dev`)
...
Wrapdf supports semantic versioning with multiple Docker tags available on both Docker Hub and GitHub Container Registry:
...
Wrapdf was originally built using **HTML**, **CSS**, and **vanilla JavaScript**. As the project grew, it was migrated to a modern stack for better maintainability and scalability:
...
Wrapdf wouldn't be possible without the amazing open-source tools and libraries that power it. We'd like to extend our heartfelt thanks to the creators and maintainers of:
